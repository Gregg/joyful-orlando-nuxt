<script setup lang="ts">

useHead({
        title: "About Us – The Heart Behind Joyful Orlando",
        meta: [
                {
                        name: "description",
                        content: "Joyful Orlando was born from a passion for conscious connection, community joy, and heart-centered events. Meet the team and discover what drives our mission to spread joy across the city.",
                },
        ],
});

useSeoMeta({
        description: "Joyful Orlando was born from a passion for conscious connection, community joy, and heart-centered events. Meet the team and discover what drives our mission to spread joy across the city.",
        ogDescription: "Joyful Orlando was born from a passion for conscious connection, community joy, and heart-centered events. Meet the team and discover what drives our mission to spread joy across the city.",
        twitterTitle: "About Us – The Heart Behind Joyful Orlando",
        twitterDescription: "Joyful Orlando was born from a passion for conscious connection, community joy, and heart-centered events. Meet the team and discover what drives our mission to spread joy across the city.",
        ogImage: "/images/this-is-joyful-orlando.png",
});
</script>

<template>
        <div>
                <!--    Hero section start    -->
                <section class="innpage-hero-section" style="background-color: #62CBFB;">
                        <img src="/images/cloudwhite.webp" class="hero-left-shape" alt="">
                        <div class="container">
                                <div class="row align-items-center gx-0 mt-40">
                                        <div class="col-xl-12">
                                                <div class="hero-content text-center">
                                                        <h1>About Joyful Orlando</h1>
                                                </div>
                                                <img src="/images/white-star.webp" class="hero-star-shape" alt="">
                                        </div>
                                </div>
                        </div>
                        <img src="/images/colorscloud.webp" class="hero-right-shape" alt="">
                </section>
                <!--    Hero section end    -->

                <!--
                <section class="sec-padding">
                        <div class="container">
                                <div class="sec-heading sec-head-has-shape sec-head-center mb-0">
                                        <div class="row justify-content-center">
                                                <div class="col-xl-8  position-relative">
                                                        <h2 class="mb-3">
                                                                Welcome to Joyful Orlando!
                                                        </h2>
                                                        <p>Joyful Orlando is on a mission to spotlight conscious & wellness events that help you slow down, show up, and feel actually good (not just “pretend good” for Instagram). Joyful Orlando is on a mission to spotlight conscious & </p>
                                                        <div class="shead-shape">
                                                                <img
                                                                        src="/images/yellow-star.svg"
                                                                        alt="">
                                                                <img
                                                                        src="/images/purple-star.svg"
                                                                        class="purple-star"
                                                                        alt="">
                                                        </div>
                                                </div>
                                        </div>
                                        <div class="row">
                                                <div class="col-12">
                                                        <div class="video-content position-relative">
                                                                <img
                                                                        src="/images/video.webp"
                                                                        class="img-fluid"
                                                                        alt="">
                                                                <a
                                                                        class="video-btn"
                                                                        role="button"
                                                                        data-bs-toggle="modal"
                                                                        data-src="https://www.youtube.com/watch?v=DUiYVJIVNcA"
                                                                        data-bs-target="#myModal">
                                                                        <i class="fa fa-play" />
                                                                </a>
                                                        </div>
                                                </div>
                                        </div>
                                </div>
                        </div>
                </section>
                -->

                <!--   Mission section start   -->
                <section class="sec-padding position-relative" style="background-image: url(/images/missionbg.webp);">
                        <div class="container">
                                <div class="row align-items-center">
                                        <div class="col-xl-4 col-lg-4 col-md-12 col-sm-12">
                                                <div class="mission-left" style="display: flex; justify-content: center; align-items: center; height: 100%;">
                                                        <img src="/images/aboutus-freehugs.webp" alt="Free Hugs" class="img-fluid">
                                                </div>
                                        </div>
                                        <div class="col-xl-8 col-lg-8 col-md-12 col-sm-12 align-content-center">
                                                <div class="mission-content-right ">
                                                        <div class="title-content">
                                                                <h2>Our Mission</h2>
                                                        </div>
                                                        <p>
                                                                Our mission is to spotlight Orlando’s vibrant conscious and wellness community — from breathwork and
                                                                meditation to dance, sound healing, and authentic connection. We believe these spaces help people slow
                                                                down, tune in, and cultivate a more meaningful, joyful life. <br><br>
                                                                We’re not just curating events; we’re building a movement toward a more connected, compassionate Orlando.
                                                                By bringing people together in spaces of healing and presence, we hope to ripple well-being through
                                                                neighborhoods, relationships, and the city as a whole.
                                                        </p>
                                                        <div class="mission-steps">
                                                                <div>
                                                                        <div class="outer-box">
                                                                                <i class="fa fa-check-circle" style="color: #FE7D15;" />
                                                                                <p>Mindfulness & Meditation</p>
                                                                        </div>
                                                                        <div class="outer-box">
                                                                                <i class="fa fa-check-circle" style="color: #9F64FC;" />
                                                                                <p>Movement & Expression</p>
                                                                        </div>
                                                                </div>
                                                                <div>
                                                                        <div class="outer-box">
                                                                                <i class="fa fa-check-circle" style="color: #62CBFB;" />
                                                                                <p>Healing & Energy Work</p>
                                                                        </div>
                                                                        <div class="outer-box">
                                                                                <i class="fa fa-check-circle" style="color: #FF839F;" />
                                                                                <p>Community & Connection</p>
                                                                        </div>
                                                                </div>
                                                        </div>
                                                </div>
                                        </div>
                                </div>
                        </div>
                        <div class="right-vector">
                                <img src="/images/right-vector.webp" alt="">
                        </div>
                </section>
                <!--   Mission section end   -->

                <!--   team section start  -->

                <section class="sec-padding">
                        <div class="container">
                                <div class="sec-heading  sec-head-center">
                                        <div class="row">
                                                <div class="col">
                                                        <h2>The Joy Engineers</h2>
                                                </div>
                                        </div>
                                        <div class="row">
                                                <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 team-outer">
                                                        <div class="team-member-detail">
                                                                <div class="member-profile">
                                                                        <img src="/images/Ishaan.jpg" alt="Ishaan Vadhera">
                                                                </div>
                                                                <div class="team-detail">
                                                                        <p>Ishaan brings humor, warmth, and soulful presence to every space. He has a gift for making people
                                                                                feel seen, breaking down walls, and turning gatherings into moments of joy and authentic connection.
                                                                        </p>
                                                                        <h6>Ishaan Vadhera</h6>
                                                                        <span>Director of Human Sunshine</span>
                                                                </div>
                                                        </div>
                                                </div>
                                                <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 team-outer">
                                                        <div class="team-member-detail">
                                                                <div class="member-profile">
                                                                        <img src="/images/Gregg.jpg" alt="Gregg Pollack">
                                                                </div>
                                                                <div class="team-detail">
                                                                        <p>Gregg creates spaces where people slow down, open up, and connect deeply. With his mix of intention
                                                                                and playfulness, he’s building a more conscious, compassionate Orlando — one gathering at a time.</p>
                                                                        <h6>Gregg Pollack</h6>
                                                                        <span>Captain of Conscious Connection</span>
                                                                </div>
                                                        </div>
                                                </div>
                                        </div>
                                        <div class="row justify-content-center">
                                                <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 team-outer">
                                                        <div class="team-member-detail">
                                                                <div class="member-profile">
                                                                        <img src="/images/David.jpg" alt="David Wheeler">
                                                                </div>
                                                                <div class="team-detail">
                                                                        <p>David harnesses the power of moving pictures to open hearts and unite communities. Through <a
                                                                                        href="https://www.globalpeace360.org/">Global Peace 360°</a> and the Global Peace Film Festival, he
                                                                                spreads empathy, positivity, and a spirit of peace across Orlando and beyond.</p>
                                                                        <h6>David Wheeler</h6>
                                                                        <span>Ambassador of Peace</span>
                                                                </div>
                                                        </div>
                                                </div>
                                        </div>
                                </div>
                        </div>
                </section>
                <!--   team section end  -->
        </div>
</template>

<style scoped></style>
